...
public class CustomerSelectFarmPanel extends javax.swing.JPanel {
    ...
    public CustomerSelectFarmPanel(JPanel userProcessContainer, UserAccount account, EcoSystem system, FarmManager farmManager, JSplitPane screen) {
        ...
        maidpn.setText(farmManager.getPhone());
        maidaddress.setText(farmManager.getAddress());
        maidgender.setText(farmManager.getGender());
    ...
    public void showMenuTable() {
        ...
        if (FarmOrderDirectory.getOrderList().size() > 0) {
            for (int i = 0; i < FarmOrderDirectory.getOrderList().size(); i++) {
                model1.addRow(new Object[]{
                    ...
                    FarmOrderDirectory.getOrderList().get(i).getDate(),
        ...
        public int CheckMaidAvailibilty(String Date) {
            int a = 0;
           System.out.println(farmManager.getOrderList().size());
        if (farmManager.getOrderList().size() > 0) {
            for (int i = 0; i < farmManager.getOrderList().size(); i++) {
                 System.out.println(i);
                 System.out.println(farmManager.getOrderList().get(i).getDate());
                if(Date.equals(farmManager.getOrderList().get(i).getDate()))
                {a = a + 1;}
    ...
    private void AddButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AddButtonActionPerformed
        ...
        FarmOrderDirectory.createOrder(selectedItem, selectedPrice, date1,String.valueOf(total), time, farmManager, userAccount);
        model1.addRow(new Object[]{
            selectedItem,
            date1,
            selectedPrice,
